...
int nave1X, nave1Y, nave2X, nave2Y; // posición de la primera parte de la vibora
...
void generateFruit(){
    ...
    do{
        ...
        if(nave1X == fruitX && nave1Y == fruitY){
            validPosition= false;
            continue;
        }
...
void logic(){
    ...
    if(nave1X <=0 || nave1X>= WIDTH -1 || nave1Y <= 0 || nave1Y >= HEIGHT-1){
        printf("Game over\n");
        game_over = true;
    }
    // Colisiones de la cabeza con el cuerpo 
    for(int i =0; i < tailLen; i++){
        if(tailX[i]== nave1X && tailY[i]==nave1Y) game_over= true;
...
int main(){
    ...
    while(!game_over){
        ...
        updateTail();
        moveNave(&nave1X, &nave1Y, direction);
        logic();
        #ifdef _WIN32
        Sleep(150);
        #else
